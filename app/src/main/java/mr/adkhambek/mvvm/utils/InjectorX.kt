...
inline fun <reified C : Context> C.getMainAPI(): MainAPI {
...
fun getMainRepository(mainAPI: MainAPI): MainRepository {
    return MainRepository(mainAPI)
}

fun getSellUseCase(mainRepository: MainRepository): SellUseCase {
    return SellUseCaseImpl(mainRepository)
}

fun getLoadProductsUseCase(mainRepository: MainRepository): LoadProductsUseCase {
